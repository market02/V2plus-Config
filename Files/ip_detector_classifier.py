...
class IPGeoClassifier:
    ...
    def _geolocate_ip(self, ip):
        ...
        try:
            ...
            response = requests.get(
                f"http://ip-api.com/json/{ip}", timeout=self.timeout
            )
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for future in futures:
                ...
                try:
                    ...
                    if geo_info:
                        geo_results.append({"ip": ip, "geo": geo_info})
                except Exception as e:
    ...
    def _is_eu_jp_kr_region(self, country_code):
        ...
        return country_code.upper() in eu_countries or country_code.upper() in [
            "JP",
            "KR",
        ]
    ...
    def classify_configs_by_ip(self, configs, output_dir="."):
        ...
        for i, config_info in enumerate(configs, 1):
            ...
            if (
                "OTHER" in regions_found
                and "US_CA" not in regions_found
                and "EU_JP_KR" not in regions_found
            ):
                other_configs.append(original_config)
                print(f"  -> 添加到其他地区分类")
        ...
        # 美国/加拿大配置
        us_ca_file = os.path.join(output_dir, "US_CA.txt")
        ...
        # 欧洲/日韩配置
        eu_jp_kr_file = os.path.join(output_dir, "EU_JP_KR.txt")
...
if __name__ == "__main__":
    # 默认禁用运行；只有当明确关闭禁用开关时才会执行 main()
    disabled = os.getenv("DISABLE_IP_CLASSIFIER", "1").lower() in (
        "1",
        "true",
        "yes",
        "y",
    ...
    if disabled:
        print(
            "ip_detector_classifier.py 已被禁用（设置 DISABLE_IP_CLASSIFIER=0 可启用）"
        )
    else:
        main()
